...
This essay explores the structural parallels between autoresearch and the [Rotifer Protocol](https://rotifer.dev), an open-source evolution framework for autonomous software agents. We argue that autoresearch represents a brilliant *vertical* demonstration of evolutionary computing, while the Rotifer Protocol provides the *horizontal* infrastructure to generalize this pattern to all agent capabilities — and, crucially, to make discoveries propagate across agents rather than remaining isolated in individual experiments.
